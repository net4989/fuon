...
def stock_trend_line_db_store(db_file_path, get_db_name, stock_price_candle_cnt):
    # 1. 데이터셋 생성하기
    # db 저장폴더
    ...
    for table_name in total_table_name:
        ...
        # 지난 월봉 시뮬레이션
        month_market_sell_buy_time = {'running_stock_date': [], 'running_y_h': [], 'running_y_l': [], 'sell_or_buy': [],
        ...
        for m in range(len(df_read) - stock_price_candle_cnt):
            ...
            if (running_y_h > poly_sell_max_price):
                ...
                month_market_sell_buy_time['sell_or_buy'].append('sell')
                month_market_sell_buy_time['poly_h_gradient'].append(poly_h_gradient)
                month_market_sell_buy_time['poly_l_gradient'].append(poly_l_gradient)
                ...
                month_market_sell_buy_time['running_y_h'].append(running_y_h)
                month_market_sell_buy_time['running_y_l'].append(running_y_l)
                month_market_sell_buy_time['sell_or_buy'].append('buy')
                month_market_sell_buy_time['poly_h_gradient'].append(poly_h_gradient)
                month_market_sell_buy_time['poly_l_gradient'].append(poly_l_gradient)
                month_market_sell_buy_time['poly_sell_max_price'].append(poly_sell_max_price)
                month_market_sell_buy_time['poly_buy_min_price'].append(poly_buy_min_price)
        ...
        plt.xticks(x_index, date_str_label)
...
if __name__ == "__main__":
    ...
    stock_trend_line_db_store(db_file_path, get_db_name, stock_price_candle_cnt)