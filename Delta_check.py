...
class Delta:
    def __init__(self, haga_pass_items, output_option_data, Delta_sum_ref, Buy_Item_Max_Cnt):
        # 수신받은 콜/풋 최근 변경 데이터
        ...
        for p in range(len(self.haga_pass_items)):
            for i in range(len(self.output_option_data['code'])):
                if self.haga_pass_items[p] == self.output_option_data['code'][i]:
                        item_cnt = 0
                        Delta_sum_fu = Delta_sum + self.output_option_data['Delta'][i]
                        while (Delta_sum < self.Delta_sum_ref_90) and (Delta_sum_fu < self.Delta_sum_ref_110):
                            Delta_sum += self.output_option_data['Delta'][i]
                            Delta_sum_fu = Delta_sum + self.output_option_data['Delta'][i]
                            item_cnt += 1
                        if item_cnt != 0:
